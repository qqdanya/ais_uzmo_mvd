...
from .admin_employee_core import (
    ...
    format_writable_departments_summary,
    format_writable_organs_summary,
    format_organs_summary,
    has_all_departments_access,
    has_all_organs_access,
    has_all_writable_departments_access,
    has_all_writable_organs_access,
    has_full_access,
    last_seen_display,
    pagination_fields,
    profile_for,
...
def employee_detail_context(request, pk):
    ...
    return {
        ...
        "writable_organs": list(profile.writable_organs.all()) if profile else [],
        "writable_departments": list(profile.writable_departments.all()) if profile else [],
        "all_organs": has_all_organs_access(user, profile),
        "all_departments": has_all_departments_access(user, profile, total_departments),
        "all_writable_organs": has_all_writable_organs_access(user, profile),
        "all_writable_departments": has_all_writable_departments_access(user, profile, total_departments),
        ...
        "no_writable_departments": not bool(profile and profile.writable_departments.exists()),
        "no_writable_organs": not bool(profile and profile.writable_organs.exists()),
        "has_full_access": has_full_access(user, profile),
        "recent_logs": logs,
